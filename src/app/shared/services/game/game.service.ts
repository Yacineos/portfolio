...
export class GameService implements GameInterface {
  ...
  // je dois produire un observable qui :
  // * s'actualise toutes les 500ms , qui incrémente le score suivant la difficulté :
  // * où on peut dériver cet observable pour faire spawn des projectiles 
  // *    
  ...
  inflictDamage(): void {
    if (this.sigGameState().health <= 0) {
      throw Error(
        'player have no health left , damage should not be inflicted '
      );
    }
  ...
  setDifficulty(difficulty: Difficulty): void {
    if (!Object.values(Difficulty).includes(difficulty)) {
      throw Error(
        'the entry is not a Difficulty , please enter a difficulty from (easy , normal, hard)'
      );
    }
  ...
  setHighScore(): void {
    if (this.sigGameState().score < this.sigGameState().highScore) {
      throw Error(
        'the current score is not superior to the current high score '
      );
    }
